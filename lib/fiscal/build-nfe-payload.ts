...
function modFromEnvironment(environment?: string | null) {
  return String(environment ?? "").toUpperCase() === "PROD" ? 1 : 2;
}

function buildDest(client: BuildNfeArgs["client"], useTradeName?: boolean | null) {
  if (!client) throw new Error("Cliente ausente para NF-e.");
  ...
  if (!isCnpj && !isCpf) {
    throw new Error("Documento do destinatário inválido para NF-e.");
  }

  if (!client.address?.cityCodeIbge || digits(client.address.cityCodeIbge).length !== 7) {
    throw new Error("Destinatário sem cMun IBGE válido.");
  }

  return {
    ...(isCnpj ? { CNPJ: doc } : { CPF: doc }),
    xNome: (useTradeName ? client.tradeName : null) || client.name || "",
    ...(client.ie ? { IE: client.ie } : {}),
    ...(client.email ? { email: client.email } : {}),
    enderDest: {
      xLgr: client.address?.street || "",
      nro: client.address?.number || "S/N",
      xBairro: client.address?.district || "",
      cMun: digits(client.address?.cityCodeIbge),
      xMun: client.address?.city || "",
      UF: client.address?.state || "",
      CEP: digits(client.address?.zip),
      cPais: "1058",
      xPais: "BRASIL",
      ...(client.phone ? { fone: digits(client.phone) } : {}),
    },
  };
}

function buildEmit(args: BuildNfeArgs) {
...
function buildItems(items: BuildNfeArgs["items"]) {
  let totalProdutos = 0;
...
export function buildNfePayload(args: BuildNfeArgs) {
  ...
  const confirmedAt = nowIso(args.order.confirmedAt);
  ...
  return {
    infNFe: {
      ...
      ide: {
        ...
        tpImp: 1,
        tpEmis: 1,
        tpAmb: modFromEnvironment(args.config?.environment),
        finNFe: 1,
        indFinal: 1,
        indPres: 1,
        procEmi: 0,
